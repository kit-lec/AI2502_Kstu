...
from t_post;
select year(now());
select name, birthday, year(birthday)
from t_emp2;


-- MySQL
select e.name                             이름,
       1 + year(now()) - year(e.birthday) 나이,
       ifnull(e.post, '')                 현재직급,
       p.post                             예상직급

from t_emp2 e
         left join t_post p
                   on 1 + year(now()) - year(e.birthday) between p.s_age and p.e_age
order by e.birthday;
...
select p.profno '교수번호', p.name '교수이름', count(q.profno) '선임수', p.hiredate '입사일'

from t_professor p
         left join t_professor q
                   on p.hiredate > q.hiredate
group by p.profno, p.name, p.hiredate
order by count(q.profno) asc;
